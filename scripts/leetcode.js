...
function parseQuestion() {
  ...
  if (checkElem(questionElem)) {
    ...
    difficultyElem = document.getElementsByClassName('css-1p5igso-Difficulty e1o5n5iy1')

    if (checkElem(difficultyElem)) {
      difficulty = difficultyElem[1].dataset.degree;
    } 
...
function parseStats() {
  const probStats = document.getElementsByClassName('css-fap0zx-ResultInfo');
...
function getNotesIfAny() {
  ...
  if (document.URL.startsWith('https://leetcode-cn.com/explore/'))
    return '';
...
const loader = setInterval(() => {
  ...
  if (probStatement !== null) {
    ...
    if (language !== null) {
      ...
      chrome.storage.local.get('stats', (s) => {
        const { stats } = s;
        const filePath = problemName + 'solution' + language;
        let sha = null;
        if (
          stats !== undefined &&
          stats.sha !== undefined &&
